# Everything Claude Code (ECC) — Agent Instructions (install overlay)

> **This file is not the upstream source.** It is copied to `~/.claude/AGENTS.md` when you run `node scripts/apply-ecc-prune.js` with `copyIntoClaudeHome` in `ecc-prune.json`.  
> **Plugin manifest overlay:** [`plugin.json`](./plugin.json) → `~/.claude/plugin.json` the same way; upstream canonical file is [`.claude-plugin/plugin.json`](../.claude-plugin/plugin.json).  
